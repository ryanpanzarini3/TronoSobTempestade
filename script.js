...
function preencherCabecalhoNosInputs() {
    const nome = document.getElementById('nomePersonagem');
    if (nome) nome.value = fichaKael.cabecalho.nomePersonagem || '';

    const classe = document.getElementById('classeNivel');
    if (classe) classe.value = fichaKael.cabecalho.classeNivel || '';

    const raca = document.getElementById('raca');
    if (raca) raca.value = fichaKael.cabecalho.raca || '';

    const antecedente = document.getElementById('antecedente');
    if (antecedente) antecedente.value = fichaKael.cabecalho.antecedente || '';

    const tendencia = document.getElementById('tendencia');
    if (tendencia) tendencia.value = fichaKael.cabecalho.tendencia || '';

    const aparencia = document.getElementById('aparencia');
    if (aparencia) aparencia.value = fichaKael.cabecalho.aparencia || '';
}

window.__applyFichaData = function (data) {
    if (!data || typeof data !== 'object') return;
    Object.assign(fichaKael, data);
    preencherCabecalhoNosInputs();
    carregarAba('inicio');
};

window.__getFichaData = function () {
    return JSON.parse(JSON.stringify(fichaKael));
};
